...
def acct_num_min(value):
        if len((value)) < 10:
            raise serializers.ValidationError(
                "Account Number should be atleast 10 digits")
        return value
class MerchantsTable(models.Model):
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    UID = models.CharField(max_length=128, unique=True,
                        primary_key=True, auto_created=True, default=uidGenerator())
    ...
    blockchainAddress = models.CharField(max_length=128, unique=True, validators=[validate_blockchainAddress])
    transaction_processing_status = models.CharField(max_length=20, blank=True, default="Avaliable")
...
class TransactionsTable(models.Model):
    ...
    transaction_narration = models.CharField(max_length=128, default=uidGenerator(), null=False, unique=True)

class XdrGeneratedTransaction(models.Model):
    merchant = models.ManyToManyField(MerchantsTable)
    status = models.CharField(max_length=128, default="submitted", blank=True, null=True)
